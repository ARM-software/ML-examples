      ...
      frameLen(frameLen)
{
    ...
    for (int i = 0; i < frameLen; i++)
        windowFunc[i] = 0.5 - 0.5*cos(M_2PI * (static_cast<float>(i)) / (frameLen));
    ...
    // Initialize FFT.
    arm_rfft_fast_init_f32(&rfft, frameLenPadded);
}
...
std::vector<float> MFCC::CreateDctMatrix(int32_t inputLength, int32_t coefficientCount)
{
    ...
    float normalizer;
    arm_sqrt_f32(2.0 / static_cast<float>(inputLength), &normalizer);
    for (k = 0; k < coefficientCount; k++) {
        for (n = 0; n < inputLength; n++) {
            M[k*inputLength + n] = normalizer * cos((static_cast<double>(M_PI))/inputLength * (n + 0.5) * k);
        }
    }
    return M;
}
...
void MFCC::MfccCompute(const int16_t * audioData, float* mfccOut)
{
    ...
    // Compute FFT.
    arm_rfft_fast_f32(&rfft, frame.data(), buffer.data(), 0);
    ...
    for (bin = 0; bin < NUM_FBANK_BINS; bin++) {
        ...
        for (i = firstIndex; i <= lastIndex; i++) {
            arm_sqrt_f32(buffer[i], &sqrtData);
