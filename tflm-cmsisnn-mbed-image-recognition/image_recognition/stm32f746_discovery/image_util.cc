...
void ResizeConvertImage(tflite::ErrorReporter* error_reporter,
                        ...
                        const uint8_t* in_image, int8_t* out_image) {
  ...
  for (int y = 0; y < out_frame_height; y++) {
    for (int x = 0; x < out_frame_width; x++) {
      ...
      // convert RGB565 to RGB888 and convert from uint8 to int8
      out_image[out_img_loc] = (0xF8 & pix_hi) - 128;
      out_image[out_img_loc + 1] =
          (((0x07 & pix_hi) << 5) | ((0xE0 & pix_lo) >> 3)) - 128;
      out_image[out_img_loc + 2] = ((0x1F & pix_lo) << 3) - 128;
    }
  }
}